...
func GenerateRSAKeypair() error {
	...
	err = savePEMKey(BaseDir+"/RSA_private.pem", key)
	if err != nil {
		return  err
	}

	err = savePublicPEMKey(BaseDir+"/RSA_public.pem", publicKey)
	if err != nil {
		return  err
	}
...
func loadPrivateKey(path string) (Signer, error) {
	data, err := ioutil.ReadFile(path)
	if err != nil {
		fmt.Println("Read private key file error: ",err)
		return nil, err
	}
...
func RSA_Sign(message string) (string,error) {
	BaseDir:=DefaultDataDir()
	signer, err := loadPrivateKey(BaseDir+"/RSA_private.pem")
	if err != nil {
...
func RSA_Verify(message string,sig string) bool {
	BaseDir:=DefaultDataDir()
	parser, err := loadPublicKey(BaseDir+"/RSA_public.pem")
	if err != nil {
...
func SaveCA(caName string,cert []byte ) error {
	...
	log.Println("write ecert to", certOut.Name())
	certOut.Close()
	return nil
}
////////////////////////////////////////////////////////////////////////////////////
...
func ParseCommitteeID() (int, int, string, error)  {
	...
	fmt.Println(config.Committee, config.Address)
...
func CheckUserCertStandard(userCert string, addr common.Address, signature []byte) error{
	...
	default:
		fmt.Println("address signature verify passed")
	}

	//err = userCert2.CheckSignature(userCert2.SignatureAlgorithm, addr.Bytes(), signature)
	//err = RSA_Verify_Pub(addr.String(), hex.EncodeToString(signature), userCert2.PublicKey)
	log.Println("check address signature: ", err==nil)
	return nil
}
