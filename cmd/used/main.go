...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	///TODO: committee config should read from contract, will update it later(lyszhang)
	commitFlag, ID, privShare, err := crypto.ParseCommitteeID()
	if err != nil {
		fmt.Println("open the committee.cfg failed!")
	}
	fmt.Println(ID, commitFlag)


	///TODO:leave the space for committee, will update it later(lyszhang)
	go func() {

		cachedLastCertIDChecked := int64(1)
		...
		for ; commitFlag == committeeRole; {
			...
			// generate key
			keyIndex, _ := state.ExpandToIndex(state.UnConfirmedAddressLen, "", 0)
			resultUnConfirmedAddressLen := usechain.TxPool().State().GetState(ContractAddr, common.HexToHash(keyIndex))
			unConfirmedAddressLen := state.GetLen(resultUnConfirmedAddressLen[:])
			...
			for i := int64(0); i < unConfirmedAddressLen; i++ {
				...
				fmt.Println("res1, res2, res3", res1, res2, res3)
				...
				log.Info("A1, S1, pubSet", A1S1, shareStrSet)
				ID := "00000000000000000000000000000000000000000001"

				log.Info("The sending msg:", A1S1+certID+ID+shareStrSet)
	...
	go func() {
		...
		for ; commitFlag == verifierRole; {
			...
			if true {
				///TODO: fix bugs on GetTheLastInternalTrans
				addr, msg := core.GetTheLastInternalTrans()
				if msg == nil {
					continue
				}
				fmt.Printf("The sender is: %x\n",addr)
