...
func (self *StateDB) CheckAddrAuthenticateStat(_addr common.Address) int {
	key := CalculateStateDbIndex(_addr.Hex()[2:], "00");
...
func (self *StateDB) GetUnconfirmedAddr(ContractAddr common.Address, addrLen int64) ([]byte, error) {
	resultOneTimeAddrLen, err := QueryDataFromStateDb(self, ContractAddr, OneTimeAddrConfirmedLenIndex, "",0)
	if err != nil {
...
func (self *StateDB) GetConfirmedMainInfo(ContractAddr common.Address, keyLen int64,pos int64) (string,error) {
	...
	for  i := int64(0); i < keyLen; i++ {
		// get data string from stateDb
		confirmedMainAddr, err := QueryDataFromStateDb(self, ContractAddr, ConfirmedMainAddress, "", i)
		if err != nil {
			fmt.Println("confirmedMainAddr error: ", err)
		}
...
func (self *StateDB) GetConfirmedMainAS(ContractAddr common.Address, keyLen int64,pos int64) (string,error) {
	...
	for  i := int64(0); i < keyLen; i++ {
		...
		keyIndex, err := QueryDataFromStateDb(self, ContractAddr, CertificateAddr, hex.EncodeToString(confirmedMainAddr), 2)
		if err != nil {
			fmt.Println("GetOneTimePubSet error: ", err)
		}
